...
int s21_is_greater(s21_decimal value_1, s21_decimal value_2) {
  int res = 0;
  if (s21_is_zero_mantiss(value_1) && s21_is_zero_mantiss(value_2)) return res;
  if (!s21_check(value_1) && !s21_check(value_2)) {  // проходит чеки
    if (!s21_get_signed(value_1) && s21_get_signed(value_2)) {
      ...
      res = s21_big_simple_is_greater(big_value_1, big_value_2);
    }
  }
  return res;
}