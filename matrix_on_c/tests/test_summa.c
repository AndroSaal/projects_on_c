 ...
 * Edit the original source file "tests/test_summa.checkmk" instead.
 */
#include "test.h"
...
START_TEST(s21_sum_matrix_2) {
  ...
  ck_assert_int_eq(s21_sum_matrix(&A, &B, &result), MATH_ERROR);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}
END_TEST
...
int test_summa_main(void) {
  Suite *s1 = suite_create("summa tests");
  TCase *tc1_1 = tcase_create("summa tests");
